...
async function main() {
  await execSQL('Full Audit', `
    TRUNCATE TABLE temp_logs;
    INSERT INTO temp_logs (msg)
    SELECT 'SEG: ' || id || ' - WA: ' || whatsapp || ' - CID: ' || COALESCE(cliente_id::text, 'NULL')
    FROM public.seguimientos_fidelizacion;
    
    INSERT INTO temp_logs (msg)
    SELECT 'CL: ' || nombre_completo || ' - WA: ' || whatsapp
    FROM public.clientes WHERE whatsapp = '+573183542883';
  `);
}
