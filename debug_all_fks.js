...
async function main() {
  await execSQL('Debug All Contexts', `
    TRUNCATE TABLE temp_logs;
    INSERT INTO temp_logs (msg)
    SELECT 
        c.conname || ' (' || t.relname || ' -> ' || r.relname || ')'
    FROM 
        pg_constraint c
        JOIN pg_class t ON t.oid = c.conrelid
        JOIN pg_class r ON r.oid = c.confrelid
    WHERE 
        t.relname IN ('pedidos', 'clientes', 'seguimientos_fidelizacion', 'interacciones')
        OR r.relname IN ('pedidos', 'clientes', 'seguimientos_fidelizacion', 'interacciones');
  `);
}
