...
async function main() {
  await execSQL('Full Schema Trace', `
    TRUNCATE TABLE temp_logs;
    -- Ver columnas de clientes
    INSERT INTO temp_logs (msg) SELECT 'COL(clientes): ' || column_name || ' (' || data_type || ')' FROM information_schema.columns WHERE table_name = 'clientes';
    -- Ver columnas de pedidos
    INSERT INTO temp_logs (msg) SELECT 'COL(pedidos): ' || column_name || ' (' || data_type || ')' FROM information_schema.columns WHERE table_name = 'pedidos';
    -- Ver todas las FKs activas reales
    INSERT INTO temp_logs (msg)
    SELECT 'FK: ' || conname || ' (' || (SELECT relname FROM pg_class WHERE oid = conrelid) || ' -> ' || (SELECT relname FROM pg_class WHERE oid = confrelid) || ')'
